...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vle1 = x1 * x2 + y1 * y2;
  const vle2 = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos0 = vle1 / vle2;
...
function isPowerOfTwo(num) {
  let powerOfTwo = 0;
  for (let i = 0; powerOfTwo < num; i += 1) {
    powerOfTwo = 2 ** i;
